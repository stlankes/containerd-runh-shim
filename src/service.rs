...
use crate::{
    common::{create_runh, has_shared_pid_namespace, ShimExecutor, GROUP_LABELS, INIT_PID_FILE},
    container::Container,
    processes::Process,
    runh::{RunhContainer, RunhFactory},
    task::TaskService,
};
...
impl Shim for Service {
    type T = TaskService<RunhFactory, RunhContainer>;
    ...
    async fn start_shim(&mut self, opts: StartOpts) -> containerd_shim::Result<String> {
        ...
        // disable before starting runh. So we only need to focus on the return value
    ...
    async fn delete_shim(&mut self) -> containerd_shim::Result<DeleteResponse> {
        ...
        runh.delete(&self.id, Some(&DeleteOpts { force: true }))
            .await
            .unwrap_or_else(|e| warn!("failed to remove runh container: {}", e));
...
async fn process_exits(
    s: Subscription,
    task: &TaskService<RunhFactory, RunhContainer>,
